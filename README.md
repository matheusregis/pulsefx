...
| ----------------- | ----- | ------------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `USD-BRL-PTAX`    | BCB   | [Olinda/PTAX — CotacaoDolarPeriodo](https://olinda.bcb.gov.br/olinda/servico/PTAX/versao/v1/swagger-ui3/) | Diária     | Taxa de câmbio de referência do mercado brasileiro; é o benchmark mais usado para conversão BRL/USD, o indicador central do produto.       |
...
- **Gráfico do detalhe**: SVG desenhado à mão (`apps/web/src/components/LineChart.tsx`), sem lib de charting. Para uma série de dezenas/poucas centenas de pontos, isso evita uma dependência pesada e mantém o componente trivial de testar; se o produto precisasse de zoom/tooltip/brush, trocaria por uma lib (ex. Recharts/visx).
